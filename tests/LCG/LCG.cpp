...
TEST(LCG, can_generator_correct_seq_1) {
    ...
    std::linear_congruential_engine<std::uint_fast32_t, a, c, m> our_generator;
    LinearCongruentialGenerator<std::uint_fast32_t, a, c, m> correct_generator;
    ASSERT_EQ(our_generator(), correct_generator());
}
...
TEST(LCG, can_throw_exception_incorrect_type_int) {
    ...
    try {
        LinearCongruentialGenerator<std::int_fast64_t, a, c, m> our_generator;
...
TEST(LCG, can_throw_exception_incorrect_type_char) {
    ...
    try {
        LinearCongruentialGenerator<char32_t, a, c, m> our_generator;
    } catch (const BaseError &error) {
        EXPECT_STREQ(error.what(), "LCG can work with unsigned integers");
    }
}
...
TEST(LCG, can_throw_exception_incorrect_type_uchart) {
    ...
    try {
        LinearCongruentialGenerator<uint_fast8_t, a, c, m> our_generator;
    } catch (const BaseError &error) {
        FAIL();
    }
    SUCCEED();
...
TEST(LCG, can_throw_exception_incorrect_type_ushort) {
    ...
    try {
        LinearCongruentialGenerator<uint_fast16_t, a, c, m> our_generator;
    } catch (const BaseError &error) {
        FAIL();
    }
    SUCCEED();
...
TEST(LCG, can_throw_exception_incorrect_type_ulong) {
    ...
    constexpr std::uint_fast32_t m = 2 << 31 - 1;
    try {
        LinearCongruentialGenerator<uint_fast32_t, a, c, m> our_generator;
    } catch (const BaseError &error) {
        FAIL();
    }
    SUCCEED();
...
TEST(LCG, can_throw_exception_incorrect_type_ulonglong) {
    ...
    try {
        LinearCongruentialGenerator<uint_fast64_t, a, c, m> our_generator;
    } catch (const BaseError &error) {
        FAIL();
    }
    SUCCEED();
...
TEST(LCG, can_throw_exception_modulus) {
    ...
    try {
        LinearCongruentialGenerator<std::uint_fast64_t, a, c, m> our_generator;
    } catch (const BaseError &error) {
...
TEST(LCG, can_throw_exception_seed_more_modules) {
    ...
    constexpr std::uint_fast64_t m = 2 << 16 - 1;
    try {
        LinearCongruentialGenerator<std::uint_fast64_t, a, c, m> our_generator(2 << 16 + 1);
    } catch (const BaseError &error) {
...
TEST(LCG, can_get_correct_min_if_modulus_not_zero) {
    ...
    std::linear_congruential_engine<std::uint_fast32_t, a, c, m> our_generator;
    LinearCongruentialGenerator<std::uint_fast32_t, a, c, m> correct_generator;
    ASSERT_EQ(our_generator.min(), correct_generator.min());
}
